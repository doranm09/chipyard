...
import freechips.rocketchip.subsystem.{PeripheryBusKey, ExtMem}
...
class WithSystemModifications extends Config((site, here, up) => {
  ...
  case BootROMLocated(x) => up(BootROMLocated(x), site).map { p =>
    val pbusFreqMHz = (site(PeripheryBusKey).dtsFrequency.get / (1000 * 1000)).toLong
    val make = s"make -C fpga/src/main/resources/zcu102/sdboot PBUS_CLK=${pbusFreqMHz} bin"
