...
#define F_CLK 		(TL_CLK) // TL/periphery bus clock in MHz

// Requested SPI SCLK frequency, in kHz.
// 25MHz High Speed mode requires F_CLK >= 50MHz (SPI max SCLK is F_CLK/2).
#define SPI_CLK_TARGET 	25000
// Clamp to a realizable SCLK to avoid divider underflow on low-frequency designs.
#define SPI_CLK 	((((F_CLK) * 1000) / 2) < SPI_CLK_TARGET ? (((F_CLK) * 1000) / 2) : SPI_CLK_TARGET)
